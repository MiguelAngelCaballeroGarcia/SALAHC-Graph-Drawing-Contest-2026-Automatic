...
inline bool apply_edge_crossing_update(const Graph& graph,
    ...
    const int64_t old_penalty = weighted_penalty_for_edge(graph, edge_id, old_safe, state.lp_divisor);
    const int64_t new_penalty = weighted_penalty_for_edge(graph, edge_id, new_safe, state.lp_divisor);
...
[[nodiscard]] inline bool evaluate_node_move_incremental(const Graph& graph,
    ...
    for (const int32_t edge_id : scratch.incident_edges) {
        ...
        combined_bbox.min_x = std::min({old_a.x, old_b.x, new_a.x, new_b.x});
        combined_bbox.max_x = std::max({old_a.x, old_b.x, new_a.x, new_b.x});
        combined_bbox.min_y = std::min({old_a.y, old_b.y, new_a.y, new_b.y});
        combined_bbox.max_y = std::max({old_a.y, old_b.y, new_a.y, new_b.y});
