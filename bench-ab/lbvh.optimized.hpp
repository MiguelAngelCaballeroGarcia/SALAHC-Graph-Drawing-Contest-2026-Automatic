...
private:
    ...
    static inline uint32_t intersect_4x_avx2(const BoundingBox& target_bbox,
...
public:
    ...
    inline void query_intersections(const BoundingBox& target_bbox, F&& callback) const noexcept {
        ...
        while (stack_ptr > 0) {
            ...
            const int32_t idx0 = stack[static_cast<size_t>(--stack_ptr)];
            const int32_t idx1 = (count > 1) ? stack[static_cast<size_t>(--stack_ptr)] : idx0;
            const int32_t idx2 = (count > 2) ? stack[static_cast<size_t>(--stack_ptr)] : idx0;
            const int32_t idx3 = (count > 3) ? stack[static_cast<size_t>(--stack_ptr)] : idx0;
